#!/usr/bin/env python3
"""Audit every installed ELF for CentOS 7 ABI and bundled or OS graphics dependencies."""
import pathlib
import re
import subprocess
import sys
...
system = {'libc.so.6', 'libm.so.6', 'libpthread.so.0', 'libdl.so.2',
          'librt.so.1', 'libutil.so.1', 'libresolv.so.2', 'libnsl.so.1', 'ld-linux-x86-64.so.2',
...
for path in sorted(root.rglob('*')):
    ...
    for name in needed - system:
        if not any((root / directory / name).is_file() and
                   ...
                   for directory in ('lib/kitty-runtime', 'lib/kitty-software')):
